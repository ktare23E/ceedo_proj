...
const form = useForm({
    id : props.cottage.id,
    name: props.cottage.name,
    description: props.cottage.description,
    price_per_hour: props.cottage.price_per_hour,
    capacity: props.cottage.capacity,
    status: 'available',
});
...
const submit = () => {
    Swal.fire({
        title: 'Are you sure?',
        text: "Do you want to update this floating cottage?",
        icon: 'question',
        ...
        if (result.isConfirmed) {
            form.post(route('update_cottage'), {
                onSuccess: () => {
                    Swal.fire({
                        icon: 'success',
                        title: 'Floating Cottage Updated!',
                        text: 'The floating cottage has been successfully updated.',
                        timer: 2000,
...
<template>
    <AdminLayout :title="'Edit Floating Cottage'" :header="'Edit Floating Cottage'">
        <div class="flex items-center justify-center min-h-[80vh]">
            <div class="bg-white shadow-xl rounded-2xl p-8 w-3/4 border border-gray-100 relative mt-16">
                ...
                <h2 class="text-2xl font-semibold text-gray-800 text-center mb-6">Edit Floating Cottage</h2>
